...
typedef union {
    ...
    struct {
        uint32_t flag       : 1;
        uint32_t devs       : 3;
        uint32_t last_devs  : 3;
        uint32_t no_gui     : 1;
        uint32_t hui_index  : 3;
        uint32_t close_rgb  : 1;  // 改成位域
        uint32_t val_step   : 3;
        uint32_t spe_step   : 3;        
        uint32_t reserved   : 14; // 補齊到 32 bits
    };
} confinfo_t;
confinfo_t confinfo; 
...
static uint16_t send_battery_time = 0;
...
uint8_t get_chrg= 1;
uint16_t get_batter = 100;
uint8_t get_batter_buff = 100;
uint8_t send_batter = 100;

static bool battery_low = false;
static uint16_t battery_low_timer = 0;

static bool successful_2s = false;
static uint16_t successful_timer = 0;

// static bool Reconnect_30s = false;
static uint16_t Reconnect_timer = 0;
...
#    endif
    ...
    get_batter = *md_getp_qbat();   
    get_batter = (get_batter * 100)/95;
    if (get_batter >= 100) get_batter = 100;

    get_batter_buff = get_batter;
}
...
void wireless_post_task(void) {
    ...
    if(!readPin(USB_CABLE_IN_PIN)){
        if(get_batter >= get_batter_buff){
             

        }
        else{
            get_batter_buff = get_batter;
            send_batter =  get_batter;
        }
    }
    else{
        if(get_batter <= get_batter_buff){
    ...
    if((get_batter < 1U)&&(!readPin(USB_CABLE_IN_PIN))){
    ...
    //else if((*md_getp_bat() <= 10)&&(!readPin(USB_CABLE_IN_PIN))){
    else if((get_batter <= 10)&&(!readPin(USB_CABLE_IN_PIN))){
        battery_low = true;
    }
    else if(readPin(USB_CABLE_IN_PIN)){
        battery_low = false;
    }
    ...
    send_battery_time++;
    if(send_battery_time >= 30000){
        send_battery_time = 0;        
        md_send_bat(send_batter);        
    }
...
#    define WLS_KEYCODE_EXEC(wls_dev)                                                                                          \
    ...
    switch (keycode) {
        case KC_BT1: {
            if((confinfo.devs != DEVS_BT1)&&(confinfo.devs != DEVS_BT2)&&(confinfo.devs != DEVS_BT3))
                return false; 
            confinfo.last_devs = wireless_get_current_devs();
            eeconfig_confinfo_update(confinfo.raw);
...
void wireless_devs_change_kb(uint8_t old_devs, uint8_t new_devs, bool reset) {
    ...
    switch (new_devs) {
        ...
        case DEVS_2G4: {
            if (reset) {
                rgb_matrix_wls_indicator_set(62, (RGB){RGB_GREEN}, 200, 1);
            } else {
                rgb_matrix_wls_indicator_set(62, (RGB){RGB_GREEN}, 500, 1);
            }
        } break;
        default:
            break;
...
void rgb_matrix_wls_indicator(void) {
    ...
    if (wls_rgb_indicator_timer) {
        ...
        if (timer_elapsed32(wls_rgb_indicator_timer) >= wls_rgb_indicator_interval) {
            ...
            if (wls_rgb_indicator_times <= 0) {
                ...
                if (rgb_matrix_wls_indicator_cb() != true) {
                    successful_2s = true;
                    successful_timer = timer_read32();
                    Reconnect_timer = 0;                   
                    return;
                }

                if(wls_rgb_indicator_interval == 500){
                    Reconnect_timer ++; 
                    if(Reconnect_timer >=30){
                        Reconnect_timer = 0;
                        wls_rgb_indicator_timer = 0;                  
        ...
        if (wls_rgb_indicator_times % 2) {
            rgb_matrix_set_color(68, wls_rgb_indicator_rgb.r, wls_rgb_indicator_rgb.g, wls_rgb_indicator_rgb.b);
            rgb_matrix_set_color(69, wls_rgb_indicator_rgb.r, wls_rgb_indicator_rgb.g, wls_rgb_indicator_rgb.b);
            rgb_matrix_set_color(70, wls_rgb_indicator_rgb.r, wls_rgb_indicator_rgb.g, wls_rgb_indicator_rgb.b);
            rgb_matrix_set_color(71, wls_rgb_indicator_rgb.r, wls_rgb_indicator_rgb.g, wls_rgb_indicator_rgb.b);
            rgb_matrix_set_color(72, wls_rgb_indicator_rgb.r, wls_rgb_indicator_rgb.g, wls_rgb_indicator_rgb.b);
            rgb_matrix_set_color(73, wls_rgb_indicator_rgb.r, wls_rgb_indicator_rgb.g, wls_rgb_indicator_rgb.b);
    ...
    if ((inqbat_flag)&&(wireless_get_current_devs() != DEVS_USB )) {
        // for (uint8_t i = 0; i < 6; i++) {
            // uint8_t mi_index[6] = RGB_MATRIX_BAT_INDEX_MAP;
            // if (i < (*md_getp_bat() / 10)) {
                // if (*md_getp_bat() >= 100) {
                ...
                if (get_batter >= 100) {                    
                    rgb_matrix_set_color(68, 0, 255, 255);//100
                    rgb_matrix_set_color(69, 0, 255, 255);//100
                    rgb_matrix_set_color(70, 0, 255, 255);//100
                    rgb_matrix_set_color(71, 0, 255, 255);//100   
                    rgb_matrix_set_color(72, 0, 255, 255);//100         
                    rgb_matrix_set_color(73, 0, 255, 255);//100    
                }else if (get_batter >= 80) {
                    rgb_matrix_set_color(68, 0, 255, 0);//80~99
                    rgb_matrix_set_color(69, 0, 255, 0);//80~99
                    rgb_matrix_set_color(70, 0, 255, 0);//80~99
                    rgb_matrix_set_color(71, 0, 255, 0);//80~99   
                    rgb_matrix_set_color(72, 0, 255, 0);//80~99         
                }else if (get_batter >= 60) {
                    rgb_matrix_set_color(68, 144, 238, 144);//60~79
                    rgb_matrix_set_color(69, 144, 238, 144);//60~79
                    rgb_matrix_set_color(70, 144, 238, 144);//60~79
                    rgb_matrix_set_color(71, 144, 238, 144);//60~79                 
                }else if (get_batter >= 40) {
                    rgb_matrix_set_color(68, 255, 255, 0);//40~59
                    rgb_matrix_set_color(69, 255, 255, 0);//40~59
                    rgb_matrix_set_color(70, 255, 255, 0);//40~59
                } else if (get_batter >= 20) {
                    rgb_matrix_set_color(68, 255, 128, 0);//20~39
                    rgb_matrix_set_color(69, 255, 128, 0);//20~39
                } else {
                    rgb_matrix_set_color(68, 255, 0, 0);//0~19
                }
            // } else {
            //     rgb_matrix_set_color(mi_index[i], 0x00, 0x00, 0x00);
            // }

            // md_send_bat(get_batter); 
...
bool rgb_matrix_indicators_advanced_kb(uint8_t led_min, uint8_t led_max) {
    ...
    if(battery_low){
        ...
        if(battery_low_timer >= 1000){
            battery_low_timer = 0;
        }
        else if(battery_low_timer >= 500){
            rgb_matrix_set_color(68, 255, 0, 0);//100
            rgb_matrix_set_color(69, 255, 0, 0);//100
            rgb_matrix_set_color(70, 255, 0, 0);//100
            rgb_matrix_set_color(71, 255, 0, 0);//100   
            rgb_matrix_set_color(72, 255, 0, 0);//100         
            rgb_matrix_set_color(73, 255, 0, 0);//100    
        }
        // else{ 
        //     rgb_matrix_set_color(68, 0, 0, 0);//100
        //     rgb_matrix_set_color(69, 0, 0, 0);//100
        //     rgb_matrix_set_color(70, 0, 0, 0);//100
        //     rgb_matrix_set_color(71, 0, 0, 0);//100   
        //     rgb_matrix_set_color(72, 0, 0, 0);//100         
        //     rgb_matrix_set_color(73, 0, 0, 0);//100    
        // }
    ...
    // GUI lock
    if (confinfo.no_gui) {
        rgb_matrix_set_color(72, 0x77, 0x77, 0x77);
        rgb_matrix_set_color(73, 0x77, 0x77, 0x77); 
    } 

    //CAPS lock
    if (host_keyboard_led_state().caps_lock) {      
        rgb_matrix_set_color(68, 0x77, 0x77, 0x77);
        rgb_matrix_set_color(69, 0x77, 0x77, 0x77);
    }

    //Layer LED
    if ((get_highest_layer(layer_state|default_layer_state) == 1)||(get_highest_layer(layer_state|default_layer_state) == 3)) {
       rgb_matrix_set_color(70, 0x77, 0x77, 0x77);
       rgb_matrix_set_color(71, 0x77, 0x77, 0x77);
    }

    if (successful_2s) {
        if (timer_elapsed32(successful_timer) >= 2000){
           successful_2s = false;
        }
    ...
    if ((get_highest_layer(default_layer_state | layer_state) == 2)|| (get_highest_layer(default_layer_state | layer_state) == 3)) {
        switch (confinfo.devs) {
            case DEVS_BT1: {
                rgb_matrix_set_color(IM_MM_RGB_BLINK_INDEX_BT1,  0,130,252);
            } break;
            case DEVS_BT2: {
                rgb_matrix_set_color(IM_MM_RGB_BLINK_INDEX_BT2,  0,130,252);
            } break;
            case DEVS_BT3: {
                rgb_matrix_set_color(IM_MM_RGB_BLINK_INDEX_BT3,  0,130,252);
            } break;
            case DEVS_2G4: {
                rgb_matrix_set_color(IM_MM_RGB_BLINK_INDEX_2G4,  43,252,28);
            } break;          
            default:
            break;
        }
        ...
        if (host_keyboard_led_state().scroll_lock) {      
            rgb_matrix_set_color(54, 0x77, 0x77, 0x77);
        }
    ...
    if(confinfo.devs != DEVS_USB){ 
        ...
        else
        {
            ...
            //     }   
            //     if(bat_led_on){
            //         rgb_matrix_set_color(68, 0x77, 0, 0);                      
            //         rgb_matrix_set_color(69, 0x77, 0, 0);                 
            //         rgb_matrix_set_color(70, 0x77, 0, 0);                      
            //         rgb_matrix_set_color(71, 0x77, 0, 0);       
            //         rgb_matrix_set_color(72, 0x77, 0, 0);                      
            //         rgb_matrix_set_color(73, 0x77, 0, 0);       
            //     }
            // }    
        }
    }
